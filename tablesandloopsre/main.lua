...
function love.load()
  ...
  fruits = {"apple", "Banana"}
  --Creating a  new line ...
  shoppingList = {"socks", "coffee", "notepad"}
  addToShopping = {"1 bag", "Fishing Rod", "Corned Beef"}
  -- below is the second way of storing values in a table
  table.insert(fruits, "pear")
  table.insert(fruits, "Coconut(drupe)")
  table.insert(shoppingList, "Diapers")


  --  --adding htings to the list using loop statement
  -- for i =1,#addToShopping do
  --   v = addToShopping[i]
  --   table.insert(shoppingList, v)
  -- end

  for i, v in ipairs(addToShopping) do
    table.insert(shoppingList, v)
  end
...
function love.draw()
  ...
  for i, shopList in ipairs(shoppingList) do
    love.graphics.print(shopList, 250, 100 + 50 * i)
  end
